    ...
    public void drawInBackground(@Nonnull PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtils.useScissor(matrixStack, x, y, width, height, ()->{
            ...
            for (int i = minToRender; i < maxToRender; i++) {
                ...
                if (node.isLeaf()) {
                    if (leafTexture != null) {
                        leafTexture.draw(matrixStack, mouseX, mouseY, x, sY, width, lineHeight);
                    }
                    if (node.getContent() != null) {
                        ...
                        if (icon != null) {
                            icon.draw(matrixStack, mouseX, mouseY, sX - 2 - lineHeight, sY, lineHeight, lineHeight);
                        }
                    }
                } else {
                    if (nodeTexture != null) {
                        nodeTexture.draw(matrixStack, mouseX, mouseY, x, sY, width, lineHeight);
                    }
                ...
                if (node == selected) {
                    DrawerHelper.drawSolidRect(matrixStack, x + 1, sY, width - 2, lineHeight, 0x7f000000);
                }
                int maxWidth = Math.max(width - lineHeight * node.dimension, lineHeight - 3);
                new TextTexture(name).setType(TextTexture.TextType.LEFT_HIDE).setWidth(maxWidth).draw(matrixStack, mouseX, mouseY, sX, sY, maxWidth, lineHeight);
            }
        });
