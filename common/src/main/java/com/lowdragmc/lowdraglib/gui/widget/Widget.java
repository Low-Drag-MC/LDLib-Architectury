...
public class Widget {
    ...
    @Environment(EnvType.CLIENT)
    protected void drawBackgroundTexture(@Nonnull PoseStack poseStack, int mouseX, int mouseY) {
        if (backgroundTexture != null) {
            Position pos = getPosition();
            Size size = getSize();
    ...
    public void drawOverlay(@NotNull PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        if (overlay != null) {
            Position pos = getPosition();
            overlay.draw(poseStack, mouseX, mouseY, pos.x, pos.y, size.width, size.height);
        }
    }
